    ...
    <div class="my-4">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active" aria-current="page"> مراحل القضايا </li>
            </ol>
        </nav>
        ...
        <div class="d-flex align-items-center filt-dir">
           ...
           <!-- Button trigger modal -->
            <button type="button" class="search-btn-add" data-bs-toggle="modal" data-bs-target="#addModal">
                <span> <i class="fa-solid fa-plus"></i> </span>  إضافة مرحلة قضية 
            </button>
            ...
            <div class="modal fade" id="addModal" tabindex="-1" aria-labelledby="addModalLabel" aria-hidden="true">
                ...
                <div class="modal-content">
                    ...
                    <h1 class="modal-title fs-5" id="addModalLabel">إضافة مرحلة قضية </h1>
                    
                    </div>
                    <div class="modal-body">
                        <form action="{{route('issues_phase.store')}}" method="POST">
                            @csrf
                            <div class="mb-3">
                                <label for="name" class="form-label">مرحلة القضية</label>
        ...
        <table class="table">
            <thead>
                <tr>
                    ...
                    <th scope="col">مرحلة القضية</th>
                    <th scope="col">تاريخ الإضافة </th>
                    <th scope="col">الإجراءات</th>
                </tr>
            </thead>
            <tbody>
                @foreach($IssuesPhase as $item)
                <tr>
                ...
                <div class="modal fade" id="editModal{{$item->id}}" tabindex="-1" aria-labelledby="editModalLabel" aria-hidden="true">
                    ...
                    <div class="modal-content">
                        <div class="modal-header">
                        <h1 class="modal-title fs-5" id="editModalLabel">تعديل مرحلة قضية </h1>
                        
                        </div>
                        <div class="modal-body">
                            <form action="{{ route('issues_phase.update', $item->id) }}" method="POST">
                                @csrf
                                @method('PUT')
                                <div class="mb-3">
                                    <label for="name" class="form-label">مرحلة القضية</label>
                ...
                <div class="modal fade" id="deleteModal{{$item->id}}" tabindex="-1" aria-labelledby="deleteModal" aria-hidden="true">
                    ...
                    <div class="modal-content">
                        <div class="modal-header">
                        <h1 class="modal-title fs-5" id="deleteModal">  حذف مرحلة القضية </h1>
                        
                        </div>
                        <div class="modal-body">
                            <form action="{{ route('issues_phase.destroy', $item->id) }}" method="POST">
                                @csrf
                                @method('DELETE')
                                <h2 class="modal-title fs-5" id="deleteModal">   متأكد من حذف مرحلة القضية ؟</h2>
                                <div class="modal-footer">
                                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">إغلاق</button>
