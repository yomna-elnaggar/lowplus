    ...
    <div class="my-4">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active" aria-current="page"> انواع الضرائب </li>
            </ol>
        </nav>
    </div>
    ...
    <div class="mb-4 mob-dir">
        <div class="d-flex align-items-center filt-dir">
            ...
            <button type="button" class="search-btn-add" data-bs-toggle="modal" data-bs-target="#addModal">
                <span> <i class="fa-solid fa-plus"></i> </span>  إضافة ضريبة جديدة
            </button>
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h1 class="modal-title fs-5" id="addModalLabel"> إضافة ضريبة جديدة </h1>
                        </div>
                        <div class="modal-body">
                            <form action="{{ route('taxtype.store') }}" method="POST">
                                @csrf
                                <div class="row g-3">
                                    <div class="col-lg-12">
                                        <div>
                                            <label for="title" class="form-label">اسم الضريبة</label>
                                            <input name="title" type="text" id="title" class="form-control" placeholder="اكتب اسم الضريبة" required />
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="row g-3 mt-3">
                                    <div class="col-lg-12">
                                        <div>
                                            <label for="type" class="form-label">نوع الضريبة</label>
                                ...
                                <div class="row g-3 mt-3">
                                    <div class="col-lg-12">
                                        <div>
                                            <label for="amount" class="form-label">قيمة الضريبة</label>
                                            <input name="amount" type="number" id="amount" class="form-control" placeholder="اكتب قيمة الضريبة" required />
                                        </div>
                                    </div>
                                </div>
            ...
            <div class="filter-select d-flex">
                ...
                <label class="mb-1 mx-2 d-flex align-items-center justify-content-center"> صف لكل صفحة </label>
            </div>
        </div>
        ...
        <table class="table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <div>{{ $item->type == 'fixed' ? ' مبلغ ثابت ' : 'نسبه مئوية ' }}</div>
                    </td>
                    <td>
                        <div> {{ $item->amount }}{{ $item->type == 'percent' ? '%' : '' }}</div>
                    </td>
                    <td>
                ...
                <div class="modal fade" id="editModal{{$item->id}}" tabindex="-1" aria-labelledby="editModalLabel" aria-hidden="true">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                <form action="{{ route('taxtype.update', $item->id) }}" method="POST">
                                    @csrf
                                    @method('PUT')
                                    <div class="mb-3">
                                        <label for="title" class="form-label">اسم الضريبة </label>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select name="type" class="form-select" required>
                                            ...
                                            <option value="percent" {{ $item->type == 'percent' ? 'selected' : '' }}> نسبه مئوية </option>
                                        </select>
                                    </div>
                                    <div class="mb-3">
                                        <label for="amount" class="form-label">قيمة الضريبة</label>
                                        <input type="number" class="form-control" id="amount" value="{{ $item->amount }}" name="amount" required>
                                    </div>
                                    <div class="modal-footer">
                                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">إغلاق</button>
                ...
                <div class="modal fade" id="deleteModal{{$item->id}}" tabindex="-1" aria-labelledby="deleteModalLabel" aria-hidden="true">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h1 class="modal-title fs-5" id="deleteModalLabel"> حذف  الضريبة </h1>
                            </div>
                            <div class="modal-body">
                                <form action="{{ route('taxtype.destroy', $item->id) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <h2 class="modal-title fs-5" id="deleteModalLabel"> متأكد من حذف  الضريبة ؟</h2>
                                    <div class="modal-footer">
                                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">إغلاق</button>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        document.querySelectorAll('.status-toggle').forEach(item => {
            item.addEventListener('click', function () {
                ...
                fetch('{{ route('taxtype.updateStatus') }}', {
                    method: 'POST',
                    headers: {
