...
<div class="container table-page">
    <div class="my-4">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active" aria-current="page">المسمى الوظيفي</li>
            </ol>
        </nav>
    </div>
    ...
    <div class="mb-4 d-flex justify-content-between align-items-center">
        <div>
            <a href="{{ url('/category') }}" class="btn btn-outline-secondary">أقسام الموظفين</a>
            <a href="{{ url('/jobtitle') }}" class="btn btn-outline-secondary active">المسمى الوظيفي</a>
        </div>
        <div>
            ...
            <tbody>
                ...
                <div class="modal fade" id="editModal{{ $item->id }}" tabindex="-1" aria-labelledby="editModalLabel" aria-hidden="true">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="editModalLabel">تعديل المسمى الوظيفي</h5>
                                
                            </div>
                            <form action="{{ route('jobtitle.update', $item->id) }}" method="POST">
                                @csrf
                                @method('PUT')
                                <div class="modal-body">
                                    <div class="mb-3">
                ...
                <div class="modal fade" id="deleteModal{{ $item->id }}" tabindex="-1" aria-labelledby="deleteModalLabel" aria-hidden="true">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="deleteModalLabel">حذف المسمى الوظيفي</h5>
                                
                            </div>
                            <form action="{{ route('jobtitle.destroy', $item->id) }}" method="POST">
                                @csrf
                                @method('DELETE')
                                <div class="modal-body">
                                    <p>هل أنت متأكد من حذف هذا المسمى الوظيفي؟</p>
                                </div>
                                <div class="modal-footer">
                                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">إغلاق</button>
...
<script>
    $(document).ready(function () {
        $('input[name="checkStatus"]').change(function () {
            ...
            $.ajax({
                url: '{{ route("jobtitle.update-status") }}',
                type: 'POST',
                data: {
                    _token: '{{ csrf_token() }}',
                    id: menuId,
